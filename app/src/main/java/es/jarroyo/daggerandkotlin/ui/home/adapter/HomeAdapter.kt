 ...
 */
class HomeAdapter(private val homeList: HomeDisplayModel): RecyclerView.Adapter<HomeAdapter.ViewHolder>() {
    override fun getItemCount(): Int = homeList.size()

    override fun onBindViewHolder(holder: ViewHolder?, position: Int) {
        val homeData = homeList.get(position)
        holder?.bind(homeData, position)
    }
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(dataHome: HomeDisplayModel.DataHomeDisplayModel, position: Int) = with(itemView) {
